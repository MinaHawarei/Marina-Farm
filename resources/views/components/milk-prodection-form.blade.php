{{-- MilkProductionFormComponent.blade.php --}}
<div id="milk-form" class="fixed inset-0 z-50 {{ $isVisible ? '' : 'hidden' }} overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <!-- Backdrop -->
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-5">
                        <div class="relative">
                            ...
                            <div class="relative">
                                <select name="animal_id" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all duration-200 outline-none appearance-none" required>
                                    <option value="">-- اختر كود الحيوان --</option>
                                    @foreach($animals as $animal)
                                        <option value="{{ $animal->animal_code }}" {{ $milkRecord && $milkRecord->animal_id == $animal->animal_code ? 'selected' : '' }}>
                                            {{ $animal->animal_code }}
                                        </option>
                                    @endforeach
                                </select>
